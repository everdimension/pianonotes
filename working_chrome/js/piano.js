...
function getNote() {
  ...
  if (onlyNote == notes[6] && onlyOct == 5) {
	...
	var note_li = $(".note-sheet ul").find('li').last();

	note_li.find(".note").addClass(' note-' + onlyNote + onlyOct);
	note_li.find(".accidental").addClass(onlyAcc);
...
function removeNote(guessedNote) {
  var note_li = $(".note-sheet ul").find('li').last();
  note_li.find(".note").removeClass(' note-' + guessedNote[0] + guessedNote[1]);
  note_li.find(".accidental").removeClass(guessedNote.slice(3));
}
